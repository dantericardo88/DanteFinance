...
Environmental (E) pillar — structured EDGAR + EPA:
  - CDP disclosure tier scraped from cdp.net public scores page
...
Composite: E*0.33 + S*0.33 + G*0.34 (equal-weighted default; sector weights available)
...
class EnvironmentalPillarResult(BaseModel):
    ...
    cdp_score_pts: float = 0.0      # max 40
...
# ---------------------------------------------------------------------------
# CDP scraper — public A-list and scoring tier
# ---------------------------------------------------------------------------


class CDPScraper:
    """
    Scrape CDP public disclosure scores from cdp.net/en/scores.

    CDP publishes an annual CSV of scored companies. The public page lists
    A-list companies explicitly. We check the HTML for company names.
    Grade tiers: A / A- / B / B- / C / C- / D / D- / not_disclosed
    """
    ...
    def _refresh_cache(cls) -> None:
        ...
        if cls._CACHE_DATE == today and cls._CACHE:
            return
        try:
            time.sleep(_RATE_DELAY)
            r = requests.get(
            ...
            # CDP A-list table: look for company names in table cells
            # A-list companies are listed in a results table or search results
            for row in soup.select("table tr, .company-name, .results-item"):
                text = row.get_text(strip=True)
                if text:
                    # Store normalized company names mapped to "A" tier
                    cls._CACHE[text.upper()[:40]] = "A"
            cls._CACHE_DATE = today
        except Exception as exc:
            logger.warning("cdp_scrape_failed", error=str(exc))

    @classmethod
    def get_tier(cls, company_name: str, ticker: str) -> str:
        """
        Return CDP disclosure tier for a company.

        Strategy:
        1. Check cached A-list from cdp.net/en/scores HTML
        2. Normalize company name and ticker against known entries
        3. Fall back to "not_disclosed"
        """
        cls._refresh_cache()
        name_upper = company_name.upper()[:40]
        tick_upper = ticker.upper()
        for key, tier in cls._CACHE.items():
            if tick_upper in key or key in name_upper:
                return tier
        # Secondary: short name match (first 12 chars)
        for key, tier in cls._CACHE.items():
            if key[:12] in name_upper:
                return tier
...
class EnvironmentalPillar:
    ...
    def score(
        ...
        # ── 1. CDP tier (0–40 pts) ────────────────────────────────────────────
        tier = CDPScraper.get_tier(company_name, ticker)
        result.cdp_tier      = tier
        result.cdp_score_pts = CDP_TIER_SCORES.get(tier, 0.0)
        details["cdp_tier"]  = tier
        details["cdp_pts"]   = result.cdp_score_pts

        # ── 2. GHG emissions from XBRL (0–20 pts for trend) ──────────────────
        ...
        # Extract latest and prior year values
        s1_latest = s1_prior = s2_latest = s2_prior = None
        if scope1_series:
            # Filter to annual 10-K filings only
            annual = [f for f in scope1_series if "10-K" in f.get("form", "")]
            if annual:
                s1_latest = annual[0].get("value")
                s1_prior  = annual[1].get("value") if len(annual) > 1 else None
        ...
        # ── 4. Climate goals from 10-K text (0–20 pts) ───────────────────────
        goals_score = 0.0
        goals_found = {}
        if filing_text:
        ...
        # ── 5. Item 1C presence check (SEC 2024 climate rule) ────────────────
        item_1c = False
        if filing_text:
        ...
        raw = (result.cdp_score_pts +
               result.emissions_trend_score +
               result.climate_goals_score +
               result.epa_penalty_pts)
        # Normalize: max raw = 40+20+20 = 80; scale to 100
